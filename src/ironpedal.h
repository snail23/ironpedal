...
namespace Snailsoft
{
    ...
    public:
        ...
        {
            return (Color[]){
                {SSD1351_get_rgb(238, 35, 145), SSD1351_get_rgb(162, 0, 69), SSD1351_get_rgb(255, 112, 222)},  // Base color: 0xEE2391 (Cruel Jewel)
                {SSD1351_get_rgb(238, 35, 43), SSD1351_get_rgb(162, 0, 0), SSD1351_get_rgb(255, 112, 120)},    // Base color: 0xEE232B (Rouge Sarde)
                {SSD1351_get_rgb(238, 129, 35), SSD1351_get_rgb(162, 53, 0), SSD1351_get_rgb(255, 206, 112)},  // Base color: 0xEE8123 (Orange Gluttony)
                {SSD1351_get_rgb(238, 231, 35), SSD1351_get_rgb(162, 155, 0), SSD1351_get_rgb(255, 255, 112)}, // Base color: 0xEEE723 (Herbery Honey)
                {SSD1351_get_rgb(144, 238, 35), SSD1351_get_rgb(68, 162, 0), SSD1351_get_rgb(221, 255, 112)},  // Base color: 0x90EE23 (Luxurious Lime)
                {SSD1351_get_rgb(43, 238, 35), SSD1351_get_rgb(0, 162, 0), SSD1351_get_rgb(120, 255, 112)},    // Base color: 0x2BEE23 (Vivid Green)
                {SSD1351_get_rgb(35, 238, 129), SSD1351_get_rgb(0, 162, 53), SSD1351_get_rgb(112, 255, 206)},  // Base color: 0x23EE81 (Emerald Ice Palace)
                {SSD1351_get_rgb(35, 238, 231), SSD1351_get_rgb(0, 162, 155), SSD1351_get_rgb(112, 255, 255)}, // Base color: 0x23EEE7 (Cold Heights)
                {SSD1351_get_rgb(35, 144, 238), SSD1351_get_rgb(0, 68, 162), SSD1351_get_rgb(112, 221, 255)},  // Base color: 0x2390EE (Button Blue)
                {SSD1351_get_rgb(35, 43, 238), SSD1351_get_rgb(0, 0, 162), SSD1351_get_rgb(112, 120, 255)},    // Base color: 0x232BEE (Delft Blue)
                {SSD1351_get_rgb(129, 35, 238), SSD1351_get_rgb(53, 0, 162), SSD1351_get_rgb(206, 112, 255)},  // Base color: 0x8123EE (Purple Blue)
                {SSD1351_get_rgb(231, 35, 238), SSD1351_get_rgb(155, 0, 162), SSD1351_get_rgb(255, 112, 255)}  // Base color: 0xE723EE (Apple Ii Magenta)
            }[this->storage->GetSettings().color];
        }
        ...
        {
            char buf[16];
            SSD1351_write_string(this->GetColor().base, this->font, "\n");

            uint8_t y = SSD1351_cursor.y - this->font.height / 2 - 1;
            SSD1351_draw_line(0, y, COLUMNS, y, this->GetColor().light);
            ...
            SSD1351_write_string(this->GetColor().base, this->font, buf, ALIGN_CENTER);

            SSD1351_write_string(this->GetColor().light, this->font, effect_name, ALIGN_CENTER);
            ...
            if (this->current_effect.id == Effect::EFFECT_MASTER || this->current_effect.id == Effect::EFFECT_MISC)
            {
                if (this->GetEffect(this->current_effect.id).locked)
                    SSD1351_write_string(this->GetColor().dark, this->font, "LOCKED\n", ALIGN_CENTER);
            }
            else
            {
                sprintf(buf, "%s%s", this->GetEffect(this->current_effect.id).enabled ? "ON" : "OFF", this->GetEffect(this->current_effect.id).locked ? " / LOCKED\n" : "\n");
                SSD1351_write_string(this->GetColor().dark, this->font, buf, ALIGN_CENTER);
            }
        }
    ...
    private:
        ...
        void MessageBox(const char *text, uint32_t event)
        {
            ...
            SSD1351_draw_round_rect(SSD1351_cursor.x - 6, SSD1351_cursor.y - 6, this->font.width * length + 12, this->font.height + 12 - 1, 4, this->GetColor().dark);
            SSD1351_draw_round_rect(SSD1351_cursor.x - 5, SSD1351_cursor.y - 5, this->font.width * length + 10, this->font.height + 10 - 1, 4, this->GetColor().dark);

            SSD1351_write_string(this->GetColor().dark, this->font, buf);
            SSD1351_update();
        }
        ...
        void Splash()
        {
            ...
            SSD1351_draw_bitmap(0, 0, 128, 128, bitmap, this->GetColor().base);
            SSD1351_update();
